...
def is_tunnel_already_running(local_port, remote_host, remote_port):
    """
    Checks if an SSH tunnel with the given local port is already running.
    Looks for processes that include the port forwarding string in their command.
    """
    import psutil
    search_string = f"-L {local_port}:{remote_host}:{remote_port}"  # Assuming remote port matches local

    for process in psutil.process_iter(['pid', 'name', 'cmdline']):
        try:
            process_info = process.info
            if process_info['name'] == 'ssh' and process_info['cmdline']:
                if search_string in ' '.join(process_info['cmdline']):  # cmdline is a list
                    return True  # Tunnel is already running

        except (psutil.NoSuchProcess, psutil.AccessDenied, psutil.ZombieProcess):
            pass  # Ignore errors related to processes that disappear or lack of permissions

    return False  # No matching tunnel found

def create_ssh_tunnel(local_port, remote_host, remote_port, server_alias):
    """
    Creates an SSH tunnel using the -L option to forward a local port
    to a remote host and port via a specified server.

    Args:
        local_port (int): The local port to forward.
        remote_host (str): The remote host to connect to.
        remote_port (int): The remote port on the remote host.
        server_alias (str): The SSH alias or hostname of the intermediate server.
    """
    import subprocess
    ...
    try:
        ...
        if is_tunnel_already_running(local_port, remote_host, remote_port):
            print("ssh tunnel to aims group already running")
        else:
            process = subprocess.Popen(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            print(f"SSH tunnel created to aims group: {local_port} -> {remote_host}:{remote_port} via {server_alias}")
            print("Tunnel is running in the background.  Leave this script running to maintain the tunnel.")
    except Exception as e:
        print(f"An error occurred: {e}")
        print("SSH tunnel to aims group failed!")
...
def main():
    ...
    # 连接AIMS group ssh tunnel
    aims_local_port, aims_remote_host, aims_remote_port, aims_remote_alias = get_conf("AIMS_LOCAL_PORT", "AIMS_REMOTE_HOST", "AIMS_REMOTE_PORT", "AIMS_REMOTE_ALIAS")
    create_ssh_tunnel(aims_local_port, aims_remote_host, aims_remote_port, aims_remote_alias)
