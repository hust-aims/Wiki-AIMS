...
AIMS_GROUP_INTERNAL_endpoint = "http://localhost:8088/v1/chat/completions"
...
try:
    ...
    pass
# 新版配置
if AIMS_GROUP_INTERNAL_endpoint in API_URL_REDIRECT: AIMS_GROUP_INTERNAL_endpoint = API_URL_REDIRECT[AIMS_GROUP_INTERNAL_endpoint]
...
# -=-=-=-=-=-=- AIMS-GROUP-INTERNAL 对齐支持 -=-=-=-=-=-=-
for model in [m for m in AVAIL_LLM_MODELS if m.startswith("AIMS-GROUP-INTERNAL-")]:
    # 为了更灵活地接入vllm多模型管理界面，设计了此接口，例子：AVAIL_LLM_MODELS = ["vllm-/home/hmp/llm/cache/Qwen1___5-32B-Chat(max_token=6666)"]
    # 其中
    #   "AIMS-GROUP-INTERNAL-"             是前缀（必要）
    #   "mixtral-8x7b"      是模型名（必要）
    #   "(max_token=6666)"  是配置（非必要）
    try:
        _, max_token_tmp = read_one_api_model_name(model)
    except:
        logger.error(f"AIMS-GROUP-INTERNAL模型 {model} 的 max_token 配置不是整数，请检查配置文件。")
        continue
    model_info.update({
        model: {
            "fn_with_ui": chatgpt_ui,
            "fn_without_ui": chatgpt_noui,
            "can_multi_thread": True,
            "endpoint": AIMS_GROUP_INTERNAL_endpoint,
            "max_token": max_token_tmp,
            "tokenizer": tokenizer_gpt35,
            "token_cnt": get_token_num_gpt35,
        },
