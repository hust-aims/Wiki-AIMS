        ...
        while True:
            ...
            if chunk:
                try:
                    ...
                    openai_terminate = (has_choices) and (chunkjson['choices'][0].get("delta") is None or len(chunkjson['choices'][0]["delta"]) == 0)
...
def generate_payload(inputs:str, llm_kwargs:dict, history:list, system_prompt:str, image_base64_array:list=[], has_multimodal_capacity:bool=False, stream:bool=True):
    ...
    if llm_kwargs['llm_model'].startswith('vllm-') or llm_kwargs['llm_model'].startswith('AIMS-GROUP-INTERNAL-'):
        api_key = 'no-api-key'
    else:
